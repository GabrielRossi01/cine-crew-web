  ...
  template: `
    ...
    <main class="min-h-screen bg-gradient-to-b from-[#1a1a2e] to-[#0f0f1a] text-neutral-100 pt-24 pb-12 px-4 md:px-8">
      <div class="max-w-7xl mx-auto">
        <h1 class="text-3xl md:text-4xl font-bold font-sora mb-8 text-center text-white">Buscar Filmes</h1>

        <!-- Search Input -->
        <div class="max-w-3xl mx-auto mb-12 relative">
          <div class="absolute inset-y-0 left-0 pl-6 flex items-center pointer-events-none">
            <svg class="h-6 w-6 text-neutral-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
        ...
        @if (loading()) {
          ...
          <div class="text-center py-20 bg-white/10 backdrop-blur-xl rounded-3xl border border-white/20 max-w-2xl mx-auto shadow-2xl">
            <span class="text-5xl mb-4 block" aria-hidden="true">🔍</span>
            <h2 class="text-2xl font-bold mb-2">Nenhum filme encontrado</h2>
            <p class="text-neutral-400">Tente buscar por termos diferentes ou confira a ortografia.</p>
          </div>
        } @else if (!hasSearched()) {
          ...
          <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
            @for (movie of results(); track movie.tmdbId) {
              <div class="group relative rounded-2xl overflow-hidden bg-neutral-900 border border-white/10 hover:border-[#FFC250]/50 transition-all flex flex-col h-full shadow-lg">
                <!-- Poster or Fallback Gradient -->
                <div class="relative aspect-[2/3] overflow-hidden bg-gradient-to-br from-[#2B4393]/40 to-[#1a1a2e] flex items-center justify-center">
                  @if (movie.posterUrl) {
                    ...
                    <div class="flex flex-col items-center justify-center p-4 text-center">
                      ...
                      <span class="text-xs text-neutral-400 font-medium line-clamp-2">{{ movie.title }}</span>
                    </div>
                  }
                  <div class="absolute inset-0 bg-gradient-to-t from-black via-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none"></div>
                </div>

                <!-- Info & Action -->
                <div class="p-4 flex-1 flex flex-col justify-between bg-[#1a1a2e]/95 z-10">
                  <div class="mb-4">
                    <h3 class="font-bold text-white leading-tight mb-1 line-clamp-2 font-sora">{{ movie.title }}</h3>
                    @if (movie.releaseDate) {
                      <p class="text-neutral-400 text-sm">{{ movie.releaseDate | date:'yyyy' }}</p>
                    }
                  </div>

                  <button (click)="addToWatchlist(movie.tmdbId)"
                          [disabled]="watchlistSet().has(movie.tmdbId) || addingTmdbId() === movie.tmdbId"
                          [attr.aria-label]="watchlistSet().has(movie.tmdbId) ? 'Filme já está na lista' : 'Adicionar ' + movie.title + ' à lista'"
                          class="w-full py-2.5 rounded-xl text-sm font-bold transition-all min-h-[44px] flex items-center justify-center gap-2"
                          [ngClass]="watchlistSet().has(movie.tmdbId)
                    ...
                    @if (addingTmdbId() === movie.tmdbId) {
                      <span class="inline-block animate-spin rounded-full h-4 w-4 border-2 border-current border-t-transparent" role="status" aria-label="Adicionando..."></span>
